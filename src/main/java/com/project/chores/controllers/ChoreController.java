...
public class ChoreController {
	...
	public String processNewChore(@Valid @ModelAttribute("newChore") Chore newChore,
			 BindingResult result, Long id, Model model) {	
		if(result.hasErrors()) {
	...
	public String editChoreList(HttpSession session, Model model) {
		...
		Integer addChoreCount = 0;
		Integer addRewardCount = 0;
		model.addAttribute("chores", chores);
		model.addAttribute("rewards", rewards);
		model.addAttribute("addChoreCount", addChoreCount);
		model.addAttribute("addRewardCount", addRewardCount);
